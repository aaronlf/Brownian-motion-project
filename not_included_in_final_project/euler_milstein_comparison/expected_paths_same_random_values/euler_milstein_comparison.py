...
ds.random_vals1 = [random.gauss(0,1) for _ in range(10000)]
ds.random_vals2 = [random.gauss(0,1) for _ in range(10000)]
...
def compare_ev_different_random_numbers():
	...
	a.prepare_random_numbers(1)
	
	e = Euler(dt=0.005,t_max=2.5)
	e.prepare_random_numbers(2)
	
	m = Milstein(dt=0.005,t_max=2.5)
	m.prepare_random_numbers(3)
...
def show_strong_convergence():
	...
	dt_vals = list(np.arange(0.001,0.010,0.001)) + list(np.arange(0.01,0.05,0.005)) + list(np.arange(0.05,0.50,0.01)) + list(np.arange(0.5,1,0.05))
	num_threads = len(dt_vals)*2
	threads_done = 0
	total_start_time = time.time()
	
	for dt in dt_vals:
		start_time = time.time()
		e = Euler(dt=dt,t_max=5,number_of_sims=1000)
		e.prepare_random_numbers(1)
		m = Milstein(dt=dt,t_max=5,number_of_sims=1000)
		m.prepare_random_numbers(2)
		threads = [
					Thread(target=e.sim_for_comparison),
					Thread(target=m.sim_for_comparison)
					]
	...
	ax.plot(dt_vals, strong_convergence_term_e,label='Euler')
	ax.plot(dt_vals, strong_convergence_term_m,label='Milstein')
	ax.legend(loc='upper center', bbox_to_anchor=(0.5, 1.00), shadow=True, ncol=2)
