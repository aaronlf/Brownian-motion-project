...
class Euler(Analytical):
	...
	def __init__(self,s_initial=10,mu=0.05,sigma=0.10,dt=0.01,t_max=5,number_of_sims=2000):
	...
	def random_euler_term(self,s_i):
		return (self.mu * s_i * self.dt) + (self.sigma * s_i * np.sqrt(self.dt) * random.gauss(0,1))
	...
	def gbm_sim(self):
		...
		for _ in self.timesteps:
			r_term = self.random_euler_term(s_new)
			s_new = s_new + r_term
			s_vals.append(s_new)
		return s_vals	
