...
class Analytical:
	...
	def __init__(self,s_initial=10,mu=0.5,sigma=0.5,dt=0.01,t_max=2.5,number_of_sims=1000):
	...
	def main(self):
		...
		self.multiple_gbm_sims(self.number_of_sims)
		self.plot_gbm()
	...
	def plot_gbm(self):
		fig = plt.figure("Simulation of Geometric Brownian Motion, {} trials".format(self.number_of_sims))
		...
		if self.gbm_arr != []:
			...
			gbm_array = []
			for _ in range(self.number_of_sims):
				gbm_vals = self.gbm_sim()
	...
	def process_gbm_threaded(self,num_sims_per_thread=1000):
		num_threads = round(-(-self.number_of_sims // num_sims_per_thread))
		threads_list = []
		for _ in range(num_threads):
			threads_list.append(Thread(target=self.multiple_gbm_sims,args=(num_sims_per_thread,)))
	...
	def compare_mu_vals(self,gbm_array,print_res=False):
		...
		for step in np.arange(1,self.number_of_steps):
			...
			computed_mu_rel_errors.append(single_rel_error)	# NEW CODE
		...
		compared_mu_vals = {
							...
							'computed_mu_rel_errors':computed_mu_rel_errors, # NEW CODE
							'mu':computed_mu,
	...
	def sim_for_comparison(self):
		gbm_arr = self.multiple_gbm_sims(self.number_of_sims)
		self.compare_mu_vals(gbm_arr)
		self.compare_sigma_vals(gbm_arr)
