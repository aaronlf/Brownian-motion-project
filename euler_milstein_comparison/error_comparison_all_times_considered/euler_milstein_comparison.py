...
def compare_time_step():
	...
	dt_vals = list(np.arange(0.002,0.05,0.002)) + list(np.arange(0.05,0.5,0.005)) + list(np.arange(0.5,1.25,0.01))
	...
	for dt in dt_vals:
		start_time = time.time()
		a = Analytical(dt=dt,t_max=2.5)
		e = Euler(dt=dt,t_max=2.5)
		m = Milstein(dt=dt,t_max=2.5)
		threads = [
					Thread(target=a.sim_for_comparison),
					Thread(target=e.sim_for_comparison),
					Thread(target=m.sim_for_comparison)
	...
	ax.set_ylabel('Relative error in \u03BC (%)')
	ax.plot(dt_vals, analytical_mu_errors,label='Analytical')
	ax.plot(dt_vals, euler_mu_errors,label='Euler')
	ax.plot(dt_vals, milstein_mu_errors,label='Milstein')
	ax.legend(loc='upper center', bbox_to_anchor=(0.5, 1.00), shadow=True, ncol=2)
